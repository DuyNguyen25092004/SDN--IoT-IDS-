...
#     # time.sleep(1)
#     broker.cmd("mosquitto_passwd -b -c /etc/mosquitto/passwd admin admin")
...
def run():
    ...
    broker = net["hbroker"]
    # tạo passwd
    broker.cmd("mosquitto_passwd -b -c /tmp/passwd admin admin")

    # fix quyền (QUAN TRỌNG)
    broker.cmd("chmod 644 /tmp/passwd")
    ...
    """
    broker.cmd(f"echo '{config_content}' > /tmp/mosquitto.conf")

    # restart broker
    broker.cmd("pkill -f mosquitto")
    broker.cmd("mosquitto -c /tmp/mosquitto.conf -v > /tmp/mosq.log 2>&1 &")
    time.sleep(1)
    # ── Set up default routes so all hosts can reach broker ──────────────────
    for h in net.hosts:
        h.cmd(f"ip route add default via {BROKER_IP} 2>/dev/null; true")
